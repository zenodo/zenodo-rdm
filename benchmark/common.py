...
# Time we will save the session cookies before re-setting them.
SESSION_CACHE_MAX_AGE_DAYS = 3
...
def _session_cache_is_stale() -> bool:
    if not SESSION_CACHE_FILE.exists():
        return False
    age_days = (time.time() - SESSION_CACHE_FILE.stat().st_mtime) / 86400
    return age_days > SESSION_CACHE_MAX_AGE_DAYS
...
def cached_csrf_login(client, account) -> dict[str, str]:
    email, _ = account
    cache = {} if _session_cache_is_stale() else _load_session_cache()
...
def random_file(size_tier: str) -> tuple[str, bytes]:
    ...
    return f"{uuid.uuid4()}_{filename}", _urandom(size)
