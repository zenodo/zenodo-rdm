...
def test_block_triggers_notification(app, db, UserFixture, monkeypatch):
    ...
    with app.extensions["mail"].record_messages() as outbox:
        ...
        assert msg.body.startswith("Dear Zenodo user,")
...
def test_block_notify_sends_email_with_username(app, blocked_user):
    """Blocked user with a username gets addressed as @username."""
    with patch(
            "zenodo_rdm.moderation.actions.UserAggregate.get_record",
            return_value=blocked_user,
    ):
        with app.extensions["mail"].record_messages() as outbox:
            on_block_notify(user_id=blocked_user.id)

    assert len(outbox) == 1
    assert outbox[0].body.startswith("Dear @jane_doe,")
